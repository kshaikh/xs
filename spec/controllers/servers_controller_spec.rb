...
describe ServersController do
  ...
  describe "GET index" do
    it "assigns all servers as @servers" do
      Server.stub!(:find).with(:all).and_return([mock_server])
      get :index
  ...
  describe "GET show" do
    it "assigns the requested server as @server" do
      Server.stub!(:find).with("37").and_return(mock_server)
      get :show, :id => "37"
  ...
  describe "GET new" do
    it "assigns a new server as @server" do
      Server.stub!(:new).and_return(mock_server)
      get :new
      assigns[:server].should equal(mock_server)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created server as @server" do
        Server.stub!(:new).with({'these' => 'params'}).and_return(mock_server(:save => true))
